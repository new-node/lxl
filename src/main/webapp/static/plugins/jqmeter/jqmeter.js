...
(function($) {
	...
	$.jQMeter = function(elem, options) {
		...
		if(meterOrientation == 'vertical') {
			...
			$(elem).html('<div class="therm outer-therm vertical"><div class="therm inner-therm vertical"><span  class = "totalNum" style="display:none; position:absolute;right:-29px;">'+ totalNum+'</span></div></div>');
			...
			$(elem).children('.outer-therm').children('.inner-therm').animate({height : totalNum + '%'},animationSpeed);

		} else {

			$(elem).html('<div class="therm outer-therm" ><div class="therm inner-therm"><span  class = "totalNum" style="display:none;position:absolute;right:-29px;">'+totalNum+ '</span></div></div>');
			$(elem).children('.outer-therm').attr('style','width:' + width + ';height:' + height + ';background-color:' + bgColor);
			$(elem).children('.outer-therm').children('.inner-therm').attr('style','background-color:' + barColor + ';height:' + height + ';width:0');
			$(elem).children('.outer-therm').children('.inner-therm').animate({width : totalNum + '%'},animationSpeed);
		...
		if(displayTotal) {
			...
			$({ Counter: 0 }).animate({ Counter: $(elem).children('.outer-therm').children('.inner-therm').children().text() }, {
  				...
  				easing : 'swing',
  				step : function() {
   					$(elem).children('.outer-therm').children('.inner-therm').children().text((this.Counter).toFixed(2) + '%');
  				}
			});
		}

		//Add CSS
		$(elem).append('<style>.therm{height:30px;border-radius:5px;}.outer-therm{margin:20px 0;width:84%!important;}.inner-therm span {color: #fff;display: inline-block;float: right;font-family: Trebuchet MS;font-size: 20px;font-weight: bold;}.vertical.inner-therm span{width:100%;text-align:center;}.vertical.outer-therm{position:relative;}.vertical.inner-therm{position:absolute;bottom:0;}</style>');
