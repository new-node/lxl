...
function getXPositionData(warnInfoEchart, warnInfo, isCuoTai){
	...
	jQuery.i18n.properties({//加载资浏览器语言对应的资源文件
		...
		callback: function() {//加载成功后设置显示内容
			//直
			HOMEBRIDGE_STRAIGHT = $.i18n.prop('homebridge.straight');
			//合
			HOMEBRIDGE_MERGE = $.i18n.prop('homebridge.merge');
			//梁段
			HOMEBRIDGE_BEAMSEGMENT = $.i18n.prop('homebridge.beamSegment');
			//小
	...
	for (var i = 0, len = warnInfo.length; i < len; i++) {
		...
		if (LD_TYPE.BKFDC == item.ldtype) {
			...
			text = HOMEBRIDGE_STRAIGHT+'('+MEASURE_SMALL+')';//直（小）
			// 横坐标标识ID
			...
			text = HOMEBRIDGE_STRAIGHT+'('+MEASURE_LARGE+')';//直(大)
			// 横坐标标识ID
			warnInfoEchart.xIdList.push(xId);
			// 显示名称
			warnInfoEchart.shortNameList.push(text);
...
//	console.log(warnInfoEchart.xInfoList);
	...
	if(shortNameList.length < 30){
		...
		for (var i = 0,len = shortNameList.length; i < len; i++){
			...
			var item = shortNameList[i];
			//HOMEBRIDGE_STRAIGHT直、HOMEBRIDGE_MERGE合
			if(item.indexOf(HOMEBRIDGE_STRAIGHT) >=0 || item.indexOf('#') >= 0 || item.indexOf(HOMEBRIDGE_MERGE) >= 0 ){
				warnInfoEchart.dataXLd.push(item);
			}else{
				if(i % 2 == 1){
...
//			var middleLdName =  warnInfo[j].shortname;
			...
			if (xId == xIdChart) {
				...
				if (pcz != null) {
					...
					var	minEchart = {coord:[i,pcz],symbol: 'circle'};
					 maxEchart = { pier : middlePier,ldName : middleLdName,pcz:pcz, coord:[i,pcz],symbol: 'circle'};
...
function drawEchartDeltaDeviation(canvasId, warnInfoEchart) {
	...
	jQuery.i18n.properties({//加载资浏览器语言对应的资源文件
		...
		callback: function() {//加载成功后设置显示内容
			//全桥高程偏差
			HOMEBRIDGE_ALTITUDEDEVIATION = $.i18n.prop('homebridge.altitudeDeviation');
			//无数据
			HOMEBRIDGE_NODATA = $.i18n.prop('homebridge.noData');
			//平均偏差
			HOMEBRIDGE_AVERAGEDEVIATION = $.i18n.prop('homebridge.averageDeviation');
			//极限偏差
			HOMEBRIDGE_LIMITDEVIATION = $.i18n.prop('homebridge.limitDeviation');
			//高程偏差
			HOMEBRIDGE_HEIGHTDEVIATION = $.i18n.prop('homebridge.heightDeviation');
			//直
			HOMEBRIDGE_STRAIGHT = $.i18n.prop('homebridge.straight');
			//合
			HOMEBRIDGE_MERGE = $.i18n.prop('homebridge.merge');
	...
	option = {
		title:{
			text:HOMEBRIDGE_ALTITUDEDEVIATION,//'全桥高程偏差'
			top:10,
			left:'45%',
			textStyle:{
				fontSize:14,
...
/*		color:['#D7D7D7','','#E72B42'],*/
		legend: {
			data:[
				  {
					  name:HOMEBRIDGE_NODATA,//无数据
					  icon:'image://static/images/wsj.png'				
				  },{
					  name:HOMEBRIDGE_AVERAGEDEVIATION,//'平均偏差'
					  icon:'image://static/images/average.png'	
				  },{
					  name:HOMEBRIDGE_LIMITDEVIATION,//'极限偏差'
					  icon:'image://static/images/jxpc.png'	
				  }],
			itemHeight:10,
			right:'15%',
			top: 10
...
//				console.log(pointPlacePczList)
		...
		xAxis:[
				{
					type: 'category',
					name:HOMEBRIDGE_BEAMSEGMENT,//'梁段'
					nameLocation:'middle',
					nameGap:30,
					nameTextStyle:{
						color:'#797979',
		...
		yAxis: [
			{
				type : 'value',
				name:HOMEBRIDGE_HEIGHTDEVIATION+'(mm)',//高程偏差
...
function drawEchartMiddleLineDeviation(canvasId, warnInfoEchart) {
	...
	jQuery.i18n.properties({//加载资浏览器语言对应的资源文件
		...
		callback: function() {//加载成功后设置显示内容
			//全桥中线偏差
			HOMEBRIDGE_LINEDEVIATION = $.i18n.prop('homebridge.lineDeviation');
			//无数据
			HOMEBRIDGE_NODATA = $.i18n.prop('homebridge.noData');
			//偏差值
			HOMEBRIDGE_DEVIATIONVALUE = $.i18n.prop('homebridge.deviationValue');
			//极限偏差
			HOMEBRIDGE_LIMITDEVIATION = $.i18n.prop('homebridge.limitDeviation');
			//直
			HOMEBRIDGE_STRAIGHT = $.i18n.prop('homebridge.straight');
			...
			MEASURE_LARGE = $.i18n.prop('measure.large');
			//中线偏差
			HOMEBRIDGE_LINEDEVIATIONS = $.i18n.prop('homebridge.lineDeviations');
		}
	});
...
//	console.log(pczListObj);
	...
	option={
		...
		title:{
			text:HOMEBRIDGE_LINEDEVIATION,//'全桥中线偏差'
			top:10,
			left:'45%',
			textStyle:{
				fontSize:14,
		...
		legend: {
			data: [{
				...
				name: HOMEBRIDGE_DEVIATIONVALUE,//'偏差值'
				icon: 'image://static/images/pcz.png'
		...
		yAxis: [
			{
				type : 'value',
				splitNumber:1,
				name:HOMEBRIDGE_LINEDEVIATIONS+'(mm)',//中线偏差
				min :minLimit,	// 最小值
				max :maxLimit,	// 最大值
				nameLocation:'middle',
				nameGap:45,
...
function drawEchartElevationDeviation(canvasId, warnInfoEchart) {
	...
	jQuery.i18n.properties({//加载资浏览器语言对应的资源文件
		...
		callback: function() {//加载成功后设置显示内容
			//全桥立模标高偏差
			HOMEBRIDGE_VERTICALMOLDDEVIATION = $.i18n.prop('homebridge.verticalMoldDeviation');
			//无数据
			HOMEBRIDGE_NODATA = $.i18n.prop('homebridge.noData');
			//平均偏差
			HOMEBRIDGE_AVERAGEDEVIATION = $.i18n.prop('homebridge.averageDeviation');
			...
			HOMEBRIDGE_LIMITDEVIATION = $.i18n.prop('homebridge.limitDeviation');
			//立模标高偏差
			HOMEBRIDGE_VERTICALMOLDDEVIATIONS = $.i18n.prop('homebridge.verticalMoldDeviations');
			//直
			HOMEBRIDGE_STRAIGHT = $.i18n.prop('homebridge.straight');
			//合
			HOMEBRIDGE_MERGE = $.i18n.prop('homebridge.merge');
	...
	option = {
		title:{
			text:HOMEBRIDGE_VERTICALMOLDDEVIATION,//'全桥立模标高偏差'
			top:10,
			left:'45%',
			textStyle:{
				fontSize:14,
		...
		yAxis: [
			{
				type : 'value',
				splitNumber:1,
				name:HOMEBRIDGE_VERTICALMOLDDEVIATIONS+'(mm)',//立模标高偏差
				min : minLimit,
				max : maxLimit,
				nameLocation:'middle',
				nameGap:45,
...
function drawEchartCuoTai(canvasId, warnInfoEchart,tsList){
	...
	jQuery.i18n.properties({//加载资浏览器语言对应的资源文件
		...
		callback: function() {//加载成功后设置显示内容
			//全桥梁段错台
			HOMEBRIDGE_WRONGTABLE = $.i18n.prop('homebridge.wrongTable');
			//错台
			HOMEBRIDGE_WRONGTAIWAN = $.i18n.prop('homebridge.wrongTaiwan');
			//错台值
			HOMEBRIDGE_WRONGTABLEVALUE = $.i18n.prop('homebridge.wrongTableValue');
			//梁段
			HOMEBRIDGE_BEAMSEGMENT = $.i18n.prop('homebridge.beamSegment');
		}
	});
	...
	var option = {
		title : {
			text : HOMEBRIDGE_WRONGTABLE,//'全桥梁段错台'
			top : 10,
			left : '45%',
			textStyle : {
				fontSize : 14,
		...
		legend : {
			data : [ {
				name : HOMEBRIDGE_WRONGTAIWAN,//'错台'
				icon : 'image://static/images/gc.png'
			}/*,{
				  name:'极限偏差',
				  icon:'image://static/images/jxpc.png'
		} */],
		...
		yAxis : [ {
			type : 'value',
			splitNumber : 4,
			name : HOMEBRIDGE_WRONGTABLEVALUE+'(mm)',//错台值
			min : limitLower ,
			max : limitUpper ,
			nameLocation : 'middle',
			nameGap : 50,
